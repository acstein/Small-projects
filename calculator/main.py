# Python program to make a simple calculator with GUI
# based on instructions on geeksforgeeks.org
...
def press(num):
    """
    Updates expression in text entry
    :param num: Text entry
...
if __name__ == "__main__":
    ...
    gui = Tk()  # Create GUI window

    gui.configure(background="blue")  # set GUI background
    gui.title("Alice's calculator")  # set GUI title
    gui.geometry("270x150")  # set GUI size

    equation = StringVar()  # variable class - create instance of this class
    expression_field = Entry(gui, textvariable=equation)  # text entry box to show expression
    expression_field.grid(row = 1, column = 0, columnspan = 4) #, ipdax=70)  # place widgets in table-like structure

    # Create buttons and place a particular locations
    # When users press the button, command or function is excecuted
